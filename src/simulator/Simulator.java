...
public class Simulator extends Group {
	 ...
	 */
	public final void buildListViewWindow() { // final because of its use in
												// the constructor
		final double PREF_WIDTH = 700.0;
		ListView<Actor> 
		listView = new ListView<Actor>(this.forcesOfLight.getCollectionActors());
	 ...
	 */
	public final void buildTableViewWindow() { // final because of its use in
												// the constructor
		final double PREF_WIDTH = 470.0;
		final double PREF_HEIGHT = 450.0;
		VBox vBoxLightArmy = new VBox
		(10.0, new Text(this.forcesOfLight.getName()), this.forcesOfLight.getTableViewOfActors());
		vBoxLightArmy.setPrefWidth(PREF_WIDTH);
		VBox vBoxDarkArmy = new VBox
		(10.0,new Text(this.forcesOfDarkness.getName()),this.forcesOfDarkness.getTableViewOfActors());

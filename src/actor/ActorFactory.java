...
/**
 * class ActorFactory is a wrapper class that encapsulates tools needed for the
 * automated generation of Actor subclass objects. The use of this class frees
 * the <i>Army</i> class from having to know any details about <i>Actor</i> or
 * its subclasses. An <i>Actor</i> object can be created using an explicit type
 * name (such as HOBBIT) or through random selection. The availability of
 * creation types is automatically presented to the programmer when using the
 * <i>ActorFactory</i>.
 * 
 * @author Rex Woollard
 * @see Actor

...
public abstract class Actor implements Serializable
{
	 ...
	 * This is used to return the army it belongs to.
	 */
	 ...
	 * @param name
	 * Used to assign a name to an <i>Actor</i> object.
	 */
	 ...
	 * @param health
	 * Used to assign health points to an <i>Actor</i> object.
	 */
	 ...
	 * 
	 * @param strength
	 *            Used to assign strength points to an <i>Actor</i> object.
	 */
	 ...
	 * 
	 * @param speed
	 *            Used to assign speed points to and <i>Actor</i> object.
	 */
	 ...
	 * 
	 * @param defender
	 *            is a reference to a different <i>Actor</i> object that will be
	...
	{
		return String.format("Name:%s   Health:%.1f   Strength:%.1f   Speed:%.1f ",
  ...
  public static TableView<Actor> createTable() {
    ...
    final double PREF_WIDTH_DOUBLE = 75.0;
 	table.setPrefWidth(PREF_WIDTH_DOUBLE * 7.5);
        ...
        TableColumn<Actor, String> nameCol      = new TableColumn<>("Name");     nameCol.setCellValueFactory     (new PropertyValueFactory<Actor, String>("name"));         nameCol.setPrefWidth(PREF_WIDTH_DOUBLE);
    ...
    healthCol.setOnEditCommit(event-> { Actor a = (event.getTableView().getItems().get(event.getTablePosition().getRow())); a.setHealth(event.getNewValue()); a.resetAvatarAttributes(); });

    strengthCol.setCellFactory(TextFieldTableCell.<Actor, Double>forTableColumn(new DoubleStringConverter()));
    strengthCol.setOnEditCommit(event-> { Actor a = (event.getTableView().getItems().get(event.getTablePosition().getRow())); a.setStrength(event.getNewValue()); a.resetAvatarAttributes(); });
    
    speedCol.setCellFactory(TextFieldTableCell.<Actor, Double>forTableColumn(new DoubleStringConverter()));
    speedCol.setOnEditCommit(event-> { Actor a = (event.getTableView().getItems().get(event.getTablePosition().getRow())); a.setSpeed(event.getNewValue()); a.resetAvatarAttributes(); });
